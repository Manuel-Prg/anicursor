...
class InstalledThemesPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<InstalledThemesPage> createState() => _InstalledThemesPageState();
}
...
class _InstalledThemesPageState extends ConsumerState<InstalledThemesPage> {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref.read(installedThemesProvider.notifier).refresh(),
        ...
        children: [
          ...
          Padding(
            ...
            child: SearchBar(
              ...
              leading: const Icon(Icons.search, size: 20, color: Colors.white38),
              onChanged: (value) => setState(() => _searchQuery = value),
              backgroundColor: WidgetStatePropertyAll(
                themeData.colorScheme.surfaceContainerHighest.withValues(alpha: 0.3),
              ),
                ...
                if (filteredThemes.isEmpty) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.mouse_outlined, size: 64, color: themeData.colorScheme.onSurfaceVariant.withValues(alpha: 0.1)),
                        const SizedBox(height: 16),
                        Text(
                          _searchQuery.isEmpty 
                              ? 'No se detectaron temas instalados' 
                              : 'No se encontraron temas que coincidan con "$_searchQuery"',
                          style: TextStyle(color: themeData.colorScheme.onSurfaceVariant.withValues(alpha: 0.5)),
                        ),
                      ],
                    ),
                  );
  ...
  Future<void> _applyTheme(BuildContext context, WidgetRef ref, InstalledTheme theme) async {
    final success = await ref.read(installedThemesProvider.notifier).apply(theme);
    if (context.mounted) {
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, InstalledTheme theme) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('¿Eliminar tema?'),
        content: Text('¿Estás seguro de que deseas eliminar "${theme.displayName}" de forma permanente?'),
        actions: [
