...
class DebugConverter {
  ...
  Future<void> run() async {
    final file = File(
      '/home/manuelprz/Documentos/cursores/Roxy_Cursor/07-Precision.ani',
    );
    if (!file.existsSync()) {
      if (kDebugMode) {
    ...
    while (true) {
      ...
      for (int s in [24, 32, 48]) {
        ...
        final resR = await Process.run('convert', [
          pngPath,
          '-resize',
          '${s}x$s!',
          'PNG32:$resized',
    ...
    final xres = await Process.run('xcursorgen', [
      confPath,
      'scratch/debug_precision.cursor',
    ]);
    if (kDebugMode) {
